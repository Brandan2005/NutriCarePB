...
export default function LoginScreen({ onGoRegister }: { onGoRegister: () => void }) {
  ...
  // Debug útil: esto es lo que tenés que autorizar en Google Cloud cuando te tire redirect mismatch
  useEffect(() => {
    if (Platform.OS === "web") {
      // solo para debug
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (cred) {
          // listo: Firebase session queda activa
        }
      } catch (e: any) {
    ...
    >
      <View style={{ flex: 1, justifyContent: "center", padding: 18 }}>
        <View style={{ alignItems: "center", marginBottom: 18 }}>
          ...
          <Text style={{ opacity: 0.7, marginTop: 4 }}>Accedé para ver tu progreso y turnos.</Text>
        </View>
